...
export default new Command({
    ...
    run: async({ client, message }) => {
        ...
        if(!target) return message?.channel.send("Who do yo want to unmute?");
        if(target.id == message.author.id) return message.channel.send("You can't unmute yourself")
        const role = message?.guild?.roles.cache.find(i => i.name == "muted");
        if(!role) return;
        
        const userRole = target?.roles.cache.has(role.id);

        if(!userRole) return message?.channel.send("That user is not muted");
