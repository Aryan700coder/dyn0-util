...
export default new Command({
    ...
    run: async({ client, message, args }) => {
        ...
        if(!target) return message?.channel.send("Who do yo want to mute?");
        if(target.id == message.author.id) return message.channel.send("You can't mute yourself")
        const reason = args?.slice(1).join(" ") || "No reason";
        const time = args.slice(3).join(" ");
        console.log(reason, time);
        const role = message?.guild?.roles.cache.find(i => i.name == "muted") as Role;
        const memberRole = message.guild?.roles.cache.find(i => i.name == "Stoopid Members");
        if(!role) return;
        
        const userRole = target?.roles.cache.has(role.id);

        if(userRole) return message?.channel.send("That user is muted");
